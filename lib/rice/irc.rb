...
=end
  ...
  class Message
    module PATTERN
      ...
      SHORTNAME = "[#{LETTER}#{DIGIT}#{JASON}](?:[-#{LETTER}#{DIGIT}#{JASON}]*[#{LETTER}#{DIGIT}#{JASON}])?"
      HOSTNAME  = "#{SHORTNAME}(?:\\.#{SHORTNAME})*"
      ...
      NICKNAME = "[#{LETTER}#{SPECIAL}][-#{LETTER}#{DIGIT}#{SPECIAL}]{0,16}"
